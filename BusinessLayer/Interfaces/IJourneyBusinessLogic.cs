...
public interface IJourneyBusinessLogic
{
    ...
    /// </summary>
    Task<List<JourneyRes>?> GetCombinationsAsync(string origin, string destination, uint numberOfFlighs = 1);

    /// <summary>
    /// Try to get the cheapest journey
    /// </summary>
    Task<JourneyRes?> GetCheapestFlightAsync(string origin, string destination, uint numberOfFlighs = 1);
}